...
export const scrapeTrlLinks = async (languages: string[]) => {
  try {
    ...
    const linkData = languages.map((lang) =>
    ...
    for (let i = 0; i < chunks.length; i++) {
      const combinedTrls = await Promise.all(
        chunks[i].map(async (x) => {
          ...
          await page.goto(
            `https://www.lughayangu.com/${x.lang}/az/${x.letter}`
          );
          ...
          const trlLinkData = await page.evaluate(async (lang) => {
            ...
            const urls = Array.from(languageElements).map(
              (v: HTMLAnchorElement) => v.href
            );
          ...
          console.log(
            `${x.lang} - Letter: ${x.letter} - ${trlLinkData.length} trl links scraped`
          );
          await browser.close();
  ...
  try {
    const combinedTrls = await Promise.all(
      langs.map(async (lang) => {
        ...
        console.log(`${lang}- ${trlLinkData.length} trl links scraped`);
        return trlLinkData as ITranslationLinkData[];
      })
    );
