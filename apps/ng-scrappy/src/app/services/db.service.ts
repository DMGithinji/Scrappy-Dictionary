...
export class DbService {
  ...
  getLanguageData(status: LanguageStatus, limit?: number) {
    if (!limit) {
      return this.db
        .collection<ILanguage>(LANGUAGE_PATH, (ref) =>
          ref.where('status', '==', status).orderBy('wordCount', 'desc')
        )
        .valueChanges();
    }
    ...
    return this.db
      .collection<ILanguage>(LANGUAGE_PATH, (ref) =>
        ref.where('status', '==', status).limit(limit)
      )
      .valueChanges();
  }
  ...
  getLanguageWords(lang: string) {
    return this.db
      .collection<ITranslationResults>(TRANSLATIONS_PATH(lang), (ref) =>
        ref.orderBy('word', 'asc')
      )
      .valueChanges();
  }
  ...
  setLangVote(lang: ILanguage) {
    lang.votes += 1;
    this.db
      .collection<ITranslationResults>(LANGUAGE_PATH)
      .doc(lang.language)
      .update(lang);
  }
}
