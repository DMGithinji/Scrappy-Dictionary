...
export const cleanDuplicates = functions
  .https.onRequest(async (req, res) => {
    try {
      ...
      await Promise.all(langs.map(async (lang) => {
        ...
        const langTrls = await getLanguageWords(db, lang.language, 'word', 2000);
        ...
        await Promise.all(_.flatten(toDel).map(async w => {
            return repo.doc(w.id).delete();
        }));

      }));

      res.send('[cleanDuplicates]:- Completed getting translation data for langs');
