...
export const getTranslationsFromLinks = functions
  ...
  .onPublish(async (message) => {
      ...
      try {
        ...
        console.log(`[getTranslationsFromLinks] :: ${data.trlLinks.length} Links published for extraction`);
        ...
        for ( let i = 0; i < syncChunks.length; i++ ) {
          ...
          await Promise.all(asyncChunks.map(trlData => {
            done++;
        ...
        if (remainder.length) {
          console.log(`[getTranslationsFromLinks] :: Re-Publishing remainind ${remainder.length} links...`);
          await publishToPubsub({trlLinks: remainder, scrapeData: data.scrapeData} as ETLData, 'getTranslationsFromLinks');
          return;
        }
        ...
        if (data?.scrapeData?.isInitialScrape) {
          ...
          console.log(`Completed initial-scrape of  ${data.scrapeData.language} 👌😎`);
          return
        }
      } catch(e)
      {
...
async function updateDictionaryData(lang: string, wordCount: number) {
  ...
  const scrapedLang  = await collection.doc(lang).get().then(snapshot => snapshot.data());
