...
import { ScrapingMetaData, publishToPubsub } from '@ng-scrappy/backend/scrapper';
...
export const setToScraping = functions.firestore.document(LANGUAGE_PATH)
    .onUpdate(async (change) => {
      ...
      const objectID = change.after.id;
        ...
        if (newData.status === 'not-supported' && newData.votes >= 10) {
          ...
          // trigger scraping
          const data = { languages: [newData.language], isInitial: true } as ScrapingMetaData;

          console.log( `Publishing ${JSON.stringify(newData.language)} for initial scraping`);
